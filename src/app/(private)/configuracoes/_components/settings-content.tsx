...
import { BellRing, Check, ChevronRight, CircleUserRound, Clock3, ExternalLink, Link2, Package, RotateCcw, Save, Settings2, ShieldCheck, Sparkles, UsersRound, WalletCards, type LucideIcon } from "lucide-react";
...
const settingsSections = [
  ...
  { id: "ever", label: "Ever", description: "Copiloto do workspace", icon: Sparkles },
      ...
      <div className="grid min-w-0 gap-5 xl:grid-cols-[250px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 space-y-5">
          {activeSection === "workspace" ? <>
            ...
            <section className="toolbar-surface flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="brand-chip text-accent"><ShieldCheck className="h-4 w-4" /></div><div><p className="text-xs font-bold text-foreground">Dados protegidos por workspace</p><p className="mt-0.5 text-[11px] text-muted-foreground">As preferências são compartilhadas entre todas as páginas deste workspace durante a sessão.</p></div></div><Button variant="outline" size="sm" onClick={resetSettings}><RotateCcw className="h-3.5 w-3.5" /> Restaurar padrão</Button></section>
          </> : null}

          {activeSection === "ever" ? <section className="data-surface p-5 sm:p-6"><SectionIntro eyebrow="Inteligência operacional" title="Ever, seu copiloto de operação" description="Controle a disponibilidade do assistente no Header e preserve uma experiência de IA orientada ao contexto do workspace." /><div className="mt-5 space-y-3"><PreferenceRow icon={Sparkles} title="Exibir o Ever no Header" description="Permitir que a equipe abra o copiloto em qualquer tela privada do CRM." checked={settings.everEnabled} onChange={(checked) => updateSettings({ everEnabled: checked })} /></div><div className="mt-5 rounded-xl border border-accent/20 bg-accent/5 p-4 text-[11px] leading-relaxed text-muted-foreground">O Ever trabalha com os dados disponíveis nesta sessão para sugerir próximos passos. Uma conexão generativa segura deve ser configurada pela camada de serviço/API.</div></section> : null}

          {activeSection === "integracoes" ? <section className="data-surface p-5 sm:p-6"><SectionIntro eyebrow="Canais conectados" title="Integrações que alimentam o CRM" description="O estado destas conexões é compartilhado com Pedidos, Conversas e os indicadores do workspace." action={<Link href="/integracoes" className="inline-flex items-center gap-1 text-[11px] font-bold text-accent hover:underline">Gerenciar integrações <ExternalLink className="h-3 w-3" /></Link>} /><div className="mt-5 space-y-2">{connectedDefinitions.map((integration) => <IntegrationRow key={integration.id} id={integration.id} name={integration.name} logo={integration.logo} icon={integration.icon} status="Conectado" tone="success" helper={`${integration.category === "vendas" ? "Pedidos e receita" : integration.category === "relacionamento" ? "Conversas e atendimento" : "Dados operacionais"}`} />)}{pendingDefinitions.slice(0, 3).map((integration) => <IntegrationRow key={integration.id} id={integration.id} name={integration.name} logo={integration.logo} icon={integration.icon} status="Disponível" tone="neutral" helper="Pronto para conectar" />)}</div><Link href="/integracoes" className="mt-4 flex items-center justify-center gap-2 rounded-xl border border-border-subtle bg-muted/20 px-3 py-2.5 text-xs font-bold text-foreground hover:bg-muted">Ver catálogo completo <ChevronRight className="h-3.5 w-3.5" /></Link></section> : null}
          ...
          {activeSection === "notificacoes" ? <section className="data-surface p-5 sm:p-6"><SectionIntro eyebrow="Comunicação operacional" title="Alertas que merecem atenção" description="Escolha quais sinais devem aparecer no Header e orientar a equipe no dia a dia." /><div className="mt-5 space-y-3"><PreferenceRow icon={BellRing} title="Alertas de SLA" description="Avisar quando um ticket se aproxima do limite de resposta." checked={settings.slaAlerts} onChange={(checked) => updateSettings({ slaAlerts: checked })} /><PreferenceRow icon={Clock3} title="Resumo semanal" description="Receber uma síntese de pedidos, receita e conversas da semana." checked={settings.weeklyDigest} onChange={(checked) => updateSettings({ weeklyDigest: checked })} /></div></section> : null}

          <div className="flex flex-wrap items-center justify-end gap-3 border-t border-border-subtle pt-4"><p className={`mr-auto text-xs font-semibold ${saved ? "text-success" : "text-muted-foreground"}`}>{saved ? <><Check className="mr-1 inline h-3.5 w-3.5" /> Preferências salvas neste navegador.</> : "As alterações são aplicadas ao workspace atual."}</p><Button variant="accent" size="sm" onClick={handleSave}><Save className="h-3.5 w-3.5" /> Salvar alterações</Button></div>
        </section>
      </div>
    </div>
  );
