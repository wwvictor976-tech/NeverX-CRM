...
import { CheckCircle2, CreditCard, Database, ExternalLink, Mail, MessageSquare, PlugZap, Search, Settings2, Store, X } from "lucide-react";
...
const categoryLabels: Record<IntegrationCategory, { title: string; description: string }> = {
  ...
  relacionamento: { title: "Canais de relacionamento", description: "Pontos de contato para atendimento e comunicação." },
...
export function IntegrationsContent() {
  ...
  const handleOpen = (integration: Integration) => { setSelectedId(integration.id); setFeedback(null); setFormValues({}); };
  const handleClose = () => { setSelectedId(null); setFeedback(null); };
  const handleConnect = () => {
    if (!selectedIntegration) return;
    const wasConnected = connected.has(selectedIntegration.id);
    setConnected((current) => { const next = new Set(current); if (wasConnected) next.delete(selectedIntegration.id); else next.add(selectedIntegration.id); return next; });
    setFeedback(wasConnected ? "Conta desligada neste protótipo." : "Conta conectada neste protótipo. A sincronização real será ativada quando a API estiver configurada.");
  };
  ...
  const renderCategory = (category: IntegrationCategory) => {
    ...
    return (
      <section key={category} className="space-y-3">
        <SectionIntro eyebrow="Conexões" title={meta.title} description={meta.description} />
  ...
  return (
    <div className="page-frame">
      <PageIntro eyebrow="Fontes da operação" title="Integrações" description="Conecte canais de venda, atendimento e operação para relacionar pedidos, dados e conversas ao mesmo cliente." meta={<StatusPill tone={connected.size ? "success" : "neutral"}><span className="mr-1.5 h-1.5 w-1.5 rounded-full bg-current" /> {connected.size} conectadas</StatusPill>} />

      <section className="toolbar-surface flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between" aria-label="Pesquisa de integrações"><div className="flex min-w-0 items-center gap-3"><div className="brand-chip text-accent"><PlugZap className="h-4 w-4" /></div><div className="min-w-0"><p className="text-xs font-bold text-foreground">Origens de dados</p><p className="truncate text-[11px] text-muted-foreground">Escolha os sistemas que alimentam o perfil unificado do cliente.</p></div></div><div className="relative w-full sm:w-72"><Search className="pointer-events-none absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filtrar integrações..." className="auth-input h-9 pl-9 text-xs" /></div></section>

      <div className="space-y-8">{renderCategory("vendas")}{renderCategory("relacionamento")}{renderCategory("operacao")}</div>

      <Modal open={!!selectedIntegration} onClose={handleClose} title={selectedIntegration ? `Conectar ${selectedIntegration.name}` : "Conectar integração"} description={selectedIntegration?.description} footer={selectedIntegration ? <><Button variant="ghost" size="sm" onClick={handleClose}>Cancelar</Button><Button variant={connected.has(selectedIntegration.id) ? "danger" : "accent"} size="sm" onClick={handleConnect}>{connected.has(selectedIntegration.id) ? <><X className="h-3.5 w-3.5" /> Desconectar conta</> : <><CheckCircle2 className="h-3.5 w-3.5" /> Conectar conta</>}</Button></> : null}>
