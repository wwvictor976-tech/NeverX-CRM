...
export function IntegrationsContent() {
  ...
  return (
    <div className="space-y-7 pb-12">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><div className="mb-2 inline-flex items-center gap-2 text-[11px] font-medium text-muted-foreground"><PlugZap className="h-3.5 w-3.5 text-accent" /> Múltiplas fontes → CRM → visão unificada</div><h2 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Conecte o seu ecossistema</h2><p className="mt-1 max-w-2xl text-xs font-medium leading-relaxed text-muted-foreground sm:text-sm">Escolha os canais que fazem parte da sua operação. Cada integração ficará preparada para relacionar pedidos, dados e conversas ao mesmo cliente.</p></div><div className="flex items-center gap-2 rounded-xl border border-border-subtle bg-card px-3 py-2 text-xs font-semibold text-muted-foreground"><span className="h-2 w-2 rounded-full bg-accent" /> {connected.size} conectadas</div></div>

      <div className="flex flex-col gap-3 rounded-2xl border border-border-subtle bg-card p-4 shadow-card sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-xl bg-accent/10 text-accent"><PlugZap className="h-4 w-4" /></div><div><p className="text-xs font-bold text-foreground">Central de integrações</p><p className="text-[11px] text-muted-foreground">Os estados são locais no protótipo e não enviam credenciais para serviços externos.</p></div></div><div className="relative w-full sm:w-64"><SearchIcon /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Filtrar integrações..." className="auth-input h-9 pl-9 text-xs" /></div></div>
