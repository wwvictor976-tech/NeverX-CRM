...
export function IntegrationsContent() {
  ...
  const handleConnect = () => {
    ...
    if (!wasConnected) {
      const missingFields = selectedIntegration.fields.filter((field) => !formValues[`${selectedIntegration.id}-${field}`]?.trim());
      if (missingFields.length) {
        setFeedback(`Preencha ${missingFields.length === 1 ? "o campo" : "os campos"} ${missingFields.join(", ")} para conectar ${selectedIntegration.name}.`);
        return;
      }
    }
    setIntegrationConnected(selectedIntegration.id, !wasConnected);
    setFeedback(wasConnected ? "Conta desconectada do workspace." : "Conta conectada ao workspace. Pedidos, canais e indicadores passam a reconhecer esta origem.");
  };
