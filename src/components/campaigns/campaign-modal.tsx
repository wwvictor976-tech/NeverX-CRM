...
import { Check, Mail, Megaphone, MessageSquare, Tag } from "lucide-react";
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Nova campanha" description="Crie uma comunicação segmentada e mantenha o histórico associado à sua operação." footer={<><Button variant="ghost" size="sm" onClick={handleClose}>Cancelar</Button><Button variant="accent" size="sm" onClick={handleSave}><Check className="h-3.5 w-3.5" /> Guardar rascunho</Button></>}>
      <div className="space-y-4">
        ...
        <div><p className="text-xs font-semibold text-foreground">Canal de envio</p><div className="mt-2 grid gap-2 sm:grid-cols-3">{([{ label: "E-mail", icon: Mail }, { label: "WhatsApp", icon: MessageSquare }, { label: "Outro canal", icon: Tag }] as const).map(({ label, icon: Icon }) => <button key={label} type="button" onClick={() => setChannel(label)} className={`flex items-center gap-2 rounded-xl border p-3 text-left text-xs font-semibold transition-colors ${channel === label ? "border-accent bg-accent/5 text-foreground" : "border-border-subtle text-muted-foreground hover:bg-muted"}`}><Icon className="h-4 w-4" />{label}{channel === label ? <Check className="ml-auto h-4 w-4 text-accent" /> : null}</button>)}</div></div>
