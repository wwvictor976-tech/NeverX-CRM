  ...
  return (
    <Modal open={open} onClose={handleClose} title="Nova campanha" description="Crie uma comunicação segmentada e mantenha o histórico associado à sua operação." footer={<><Button variant="ghost" size="sm" onClick={handleClose}>Cancelar</Button><Button variant="accent" size="sm" onClick={handleSave}><Check className="h-3.5 w-3.5" /> Salvar rascunho</Button></>}>
      <div className="space-y-4">
        ...
        <label className="block space-y-1.5 text-xs font-semibold text-foreground">Mensagem inicial<textarea value={message} onChange={(event) => setMessage(event.target.value)} className="auth-input min-h-24 resize-y py-3" placeholder="Escreva o objetivo da campanha..." /></label>
